...
//         if (userMessage === '診断' || userMessage === '副業診断' || userMessage === '適職診断') {
//           const messages = startCareerDiagnosis(userId);
...
//           if (nextQuestionIndex < DIAGNOSIS_QUESTIONS.length) {
//             const messages = createDiagnosisQuestionMessage(nextQuestionIndex, userId);
//             await client.replyMessage(event.replyToken, messages);
//             continue;
//           } else {
//             // 診断完了 - 結果表示
...
function createMultipleChoiceMessage(questionIndex, userId) {
  ...
  const quickReplyItems = remainingOptions.map(opt => ({
    type: 'action',
    action: {
      type: 'postback',
      label: opt.text,
